...
-- Create hidden frame for Blizzard nameplates
...
local function OnAddonLoaded(self, event, addonName)
  ...
  SlashCmdList["MINIMALPLATES"] = function(msg)
    ...
    elseif msg == "preview" or msg == "test" then
      MP.DB.previewMode = not MP.DB.previewMode
      print("|cff00ff00[MinimalPlates]|r Preview mode " .. (MP.DB.previewMode and "enabled" or "disabled"))
      print("  Target any unit to see all icons, effects, and dummy auras displayed on their nameplate.")
      MP.Nameplates.RefreshAll()
    else
...
-- Initialize database defaults (can be called on load or after profile change)
function MP.InitializeDatabaseDefaults()
  -- Set defaults
  ...
  -- CRITICAL: Ensure enemy NPCs are always visible (never "hide")
  if MP.DB.enemyNPCMode == "hide" or not MP.DB.enemyNPCMode then
    MP.DB.enemyNPCMode = "bar"
  end
  ...
  MP.DB.partyMembersTextOnly = MP.DB.partyMembersTextOnly ~= false -- Force party members to text-only
  ...
  MP.DB.creatureTextPosition = MP.DB.creatureTextPosition or "middle"
  ...
  -- Target nameplate color
  MP.DB.useTargetColor = MP.DB.useTargetColor or false
  MP.DB.targetColor = MP.DB.targetColor or {r = 1, g = 1, b = 0} -- Yellow
  ...
  -- REMOVED: showHealerIcon, showPetIcon, showWorldQuestIcon, showBonusObjectiveIcon
  ...
  -- CRITICAL FIX: classificationScale should be a number (1.0) not boolean
  if type(MP.DB.classificationScale) ~= "number" then
    MP.DB.classificationScale = 1.0  -- Default scale for classification text
  end
